...
function initializeLoggingState() {
  ...
  if (logParam === "true") {
    ...
    logToConsole("info", `ttmd cloud backup version ${VERSION}`);
  }
  ...
  try {
    ...
    if (data.Contents.length > 0) {
      const todaysBackupFile = data.Contents.find(
        (file) =>
          file.Key === `ttmd-backup-${currentDateSuffix}.json` ||
          file.Key === `ttmd-backup-${currentDateSuffix}.zip`
      );
      if (!todaysBackupFile) {
        ...
        const jszip = await loadJSZip();
        zip = new jszip();
        zip.file(`ttmd-backup-${currentDateSuffix}.json`, backupContent, {
          compression: "DEFLATE",
          compressionOptions: {
            level: 9,
          },
        ...
        const zipKey = `ttmd-backup-${currentDateSuffix}.zip`;
        const uploadParams = {
          Bucket: bucketName,
          Key: zipKey,
          Body: compressedContent,
...
async function deriveKey(password) {
  ...
  const salt = enc.encode("ttmd-backup-salt");
